...
int colorToIntRGBA(ui.Color newColor, ui.Color oldColor) {
  ...
  if (isAlmostSameColor(
      pixelColor: newColor, checkColor: oldColor, threshold: 50)) {
...
bool isSimilarColor(
  ...
  Color beforePaintedColor = Color.fromARGB(
      img.uint32ToRed(uint32Rgba),
      img.uint32ToGreen(uint32Rgba),
      img.uint32ToBlue(uint32Rgba),
      img.uint32ToAlpha(uint32Rgba));
  Color newPaintedColor = colorFromIntRGBA(uint32Rgba);
  return beforePaintedColor.value == standardColor.value ||
      newPaintedColor.value == standardColor.value;
}
