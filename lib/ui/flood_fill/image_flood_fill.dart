...
class ImageFloodFill extends FloodFill<ui.Image, ui.Color> {
  ...
  void _floodFillUtil(ByteData bytes, int x, int y, int width, int height, ui.Color originalColor, ui.Color newColor) {
    ...
    if (!_isInside(x, y, width, height) || !isAlmostSameColor(pixelColor: oldColor, checkColor: originalColor)) return;

    // Set the node
    setPixelColor(x: x, y: y, bytes: bytes, imageWidth: width, newColor: newColor,oldColor: oldColor);
